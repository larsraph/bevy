use crate::{
    render_asset::{ExtractResult, PrepareAssetError, RenderAsset, RenderAssetPlugin},
...
pub struct ShaderBuffer {
    /// When [`None`] the buffer has been logically extracted from the MAIN_WORLD.
...
impl ShaderBuffer {
    ...
    pub fn with_size(size: usize, asset_usage: RenderAssetUsages) -> Self {
        ...
        storage.buffer_descriptor.size = size as u64;
        storage.buffer_descriptor.mapped_at_creation = false;
        storage.asset_usage = asset_usage;
        storage
    }
...
impl RenderAsset for GpuShaderBuffer {
    ...
    type MaybeTaken = Option<Vec<u8>>;

    fn maybe_taken(source_asset: &mut Self::SourceAsset) -> &mut Self::MaybeTaken {
        &mut source_asset.data
    }

    type Extracted = (Vec<u8>, wgpu::BufferDescriptor<'static>);

    fn extract(
        source_asset: &mut Self::SourceAsset,
        data: <Self::MaybeTaken as crate::render_asset::CloneOrTake>::Inner,
    ) -> ExtractResult {
        ExtractResult(data, source_asset.buffer_descriptor.clone())
    }

    fn prepare_asset(
        (data, buffer_descriptor): Self::Extracted,
        ...
        let actual_size = extracted
            ...
            .as_ref()
            .map(|d| d.len() as u64)
            .unwrap_or(extracted.buffer_descriptor.size);

        let buffer = if let Some(prev) = previous_asset
            && prev.buffer_descriptor.size == actual_size
            && prev.buffer_descriptor.usage == extracted.buffer_descriptor.usage
            && prev.buffer_descriptor.label == extracted.buffer_descriptor.label
            && (!had_data
                || extracted
                    .buffer_descriptor
                    .usage
                    .contains(BufferUsages::COPY_DST))
        {
            ...
            render_device.create_buffer_with_data(&BufferInitDescriptor {
                label: extracted.buffer_descriptor.label,
                contents: data,
                usage: extracted.buffer_descriptor.usage,
            })
        } else {
            let new_buffer = render_device.create_buffer(&extracted.buffer_descriptor);
